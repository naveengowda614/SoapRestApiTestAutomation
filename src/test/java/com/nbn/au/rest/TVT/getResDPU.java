...
public class getResDPU extends serviceBase {

    String correlationId = "CIS-getResDPU-TVT-" +prop.getEnvValue() + "-"+ Helper.UUID();
    String scenarioName = "Scenario: Verify the response for the getResDPU API request : ";

    @Test
    public void getResDpuMakeTheRequestToTheGetResDPUAPI()  {
        RestAssuredHelper.setBaseURI(request,String.format(prop.getProperty("GetResDPUUrl"),prop.getProperty("DPU_ID")));
        ...
        assertEquals(jsonResponse.get("data.id").toString().startsWith("DPU"),true);
    }
}
